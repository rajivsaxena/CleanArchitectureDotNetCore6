...
{
    internal class VerifyMobileOtpCommandHandler : IRequestHandler<AddVerifyMobileOtpCommand, int>
    {
        private readonly ILogger<VerifyMobileOtpCommandHandler> _logger;
        private readonly IMapper _mapper;
        private readonly IHprRepository _hprRegistory;

        public VerifyMobileOtpCommandHandler(IHprRepository hprRegistory, IMapper mapper, ILogger<VerifyMobileOtpCommandHandler> logger)
        {
        ...
        public async Task<int> Handle(AddVerifyMobileOtpCommand request, CancellationToken cancellationToken)
        {
            _logger.LogInformation("Adding ABDM API Response VerifyMobileOtpCommandHandler : {request.TxnId} is successfully created.", request.TxnId);
